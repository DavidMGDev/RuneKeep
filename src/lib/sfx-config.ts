...
export const SFX_VOLUME: Partial<Record<SfxId, number>> = {
  ...
  floatMenuHighlight: 0.2, // #258: -50% (the wheel shouldn't be this noisy)
 ...
 * `gain` = filling a slot (up), `lose` = depleting one (down). Tuned per owner #258 so the bang
 * lands ON the burst/fill instead of trailing it. SOUND-only — does not change the visual.
 */
export const STAT_SOUND_DELAY_MS: Record<StatKey, { gain: number; lose: number }> = {
  hp: { gain: 484, lose: 32 },
  armor: { gain: 291, lose: 117 },
