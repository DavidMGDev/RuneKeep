...
export function RuneButton({ label, onPress, kind = 'secondary', disabled, height = 48, icon, style, accessibilityLabel }: RuneButtonProps) {
  ...
  return (
    <Pressable
      ...
      <Animated.View style={anim}>
        <ChamferBox chamfer={10} fill={fill} stroke={stroke} strokeWidth={kind === 'primary' ? 0 : 1.4} style={{ height, justifyContent: 'center' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingHorizontal: 16 }}>
            {icon}
            <Text numberOfLines={1} style={{ color, fontSize: 14, fontFamily: Body.bold, letterSpacing: 1.6, textTransform: 'uppercase' }}>
              {label}
            </Text>
          </View>
        </ChamferBox>
