            ...
            renderItem={({ item }) => {
              ...
              return (
                <DmPress
                  ...
                  {({ pressed }) => (
                    <ChamferBox chamfer={12} fill={on ? 'rgba(218,162,73,0.16)' : pressed ? 'rgba(24,28,35,0.95)' : 'rgba(14,17,22,0.9)'} stroke={on ? DmRune.accent : pinned ? DmRune.accent : DmRune.line} strokeWidth={on ? 2 : pinned ? 1.6 : 1.3} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 16, paddingVertical: 15 }}>
                      {sel.selecting ? (
