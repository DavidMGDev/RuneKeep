...
export function EncounterScreen() {
  ...
  const onMemberStat = useCallback((charId: string, key: VitalKey, d: 1 | -1) => {
    const enc = encRef.current, pty = partyRef.current, f = files[charId];
    if (!enc || !pty || !f) return;
    const r = memberDelta(enc, pty, charId, key, d, memberMaxes(f));
  ...
  const onCombatantStat = useCallback((cid: string, stat: CombatantStat, d: 1 | -1) => {
    const next = updateCombatant(cid, (c) => combatantDelta(c, stat, d));
  ...
  const memberHoldEnd = useCallback((charId: string, key: VitalKey) => { logIfChanged(`m:${charId}:${key}`, 'Player', files[charId]?.name ?? 'Player', key, memberValue(charId, key)); }, [files, memberValue, logIfChanged]);
  ...
  const combatantHoldEnd = useCallback((cid: string, stat: CombatantStat) => { const r = findCombatant(cid); if (r) logIfChanged(`c:${cid}:${stat}`, r.side, r.c.name, stat, (stat === 'hp' ? r.c.hp : r.c.stress) ?? 0); }, [findCombatant, logIfChanged]);
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 90, gap: 8 }}>
          ...
          {memberAllies.map((a) => (
            <MemberPanel
              ...
              onStat={(key, d) => onMemberStat(a.charId, key, d)}
