...
function OriginBadge({ x, label, value, icon }: BadgeProps) {
  return (
    <View style={box(x, 184.7, 62.3, 30.5)}>
      ...
      <SheetText left={29} top={13} width={34} height={11} color={Rune.red} size={8} family={Body.bold} align="left">
        {value}
      </SheetText>
    </View>
  );
...
function LevelBanner({ level }: { level: number }) {
  return (
    <View style={box(331.7, 57.1, 57.5, 113.2)}>
      ...
      <SheetText left={13} top={54} width={31} height={40} color={Rune.red} size={26} family={Display.black} align="center" tabularNums>
        {level}
      </SheetText>
    </View>
  );
