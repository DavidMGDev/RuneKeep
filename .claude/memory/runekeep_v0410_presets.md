...
description: v0.41.0 - roll presets (lib/dice-presets, stored on the character file); NEVER accumulate a running total, derive it; a props snapshot of another component's state is always stale so pass a getter; setFile alone does not persist, commitFileRef does; SVG cost is node COUNT and mount/unmount, not draw
metadata:
  type: project
---
...
READ before dice-tray, roll-preset, running-total, cross-component-state, character-persistence,
svg-performance or effect-target work.
