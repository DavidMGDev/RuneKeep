...
$ver  = 'v0.15.0'                       # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.15.0 - DM Mode: run the table with Parties, Sessions and Encounters. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`n- DM MODE: a new button on the main menu flips the app into a Dungeon Master surface - the whole DM interface takes on the desaturated Golden-Gear palette, and Characters/Cards become Parties/Sessions. Tap Player Mode to switch everything back. Your mode is remembered between launches.`n- PARTIES: build any number of parties from your saved characters (pick from the roster or import a friend's hero right in the picker). Toggle each member present or absent, then Enable a party to unlock its Sessions. Your roster itself now supports folders - group characters under a named, coloured folder, rename it, re-roll its colour, or move characters in and out.`n- SESSIONS and ENCOUNTERS: pick a party from the dropdown and open its sessions; each session is a list of encounters with the active one pinned on top. New Encounter auto-adds the present party as allies and lets you add adversaries and NPCs (name plus optional HP, thresholds, stress and description). Prep fights in advance - a party's living vitals stay in sync across encounters, and completing an encounter freezes a snapshot of how it left the party.`n- HEARTBEAT STAT CONTROL: adjust any HP / Armor / Stress / Hope with a tactile pulse - tap the icon to flip between raise and lower, hold to beat out change after change (it speeds up as you hold, with a rising tick so you can count without looking), or tap the number to type an exact value. Turn on Auto-log and every change is written to the encounter's log (markdown, newest first), one entry per hold; add your own notes any time.`n- PLAYERS OVERVIEW: a Players button shows the whole party at a glance - image, name, subclass, HP, Evasion, Armor, thresholds, Stress and Hope as numbers - tap a character to expand traits, level and proficiency.`n`nSideload: enable Install unknown apps, then open the APK."
