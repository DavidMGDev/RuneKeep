...
$ver  = 'v0.10.1'                          # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.10.1 - NFC tap-to-share, on top of v0.10.0's Card Library, homebrew expansions & the .rkp file format. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`nNew in v0.10.1:`n- NEW - NFC tap-to-share (Android): hold two phones back-to-back to hand over a card or a whole expansion - no file needed. From the Card Library, open an expansion and tap NFC to send, or tap Receive NFC on the other phone. In Golden Gear Edit on a character sheet, select cards and tap Send NFC to share them straight from your deck. A received expansion you already own updates in place by version, just like file import. (Large expansions may exceed NFC's size limit - share the .rkp file for those.)`n`nFrom v0.10.0:`n- Card Library: the Cards button opens a real library - browse the full system archive (every card, plus all weapons & armor across tiers 1-4) and author your own homebrew.`n- Expansions: named, versioned bundles of homebrew cards (Ancestry with mixed-line support, Community, Domain, Subclass, Class, or generic), authored with a live preview.`n- .rkp file format: export/import characters, single cards, or whole expansions; re-importing a newer expansion updates it in place.`n- FIX - level-up past level 8 (trait marks reset every tier; no double-upgrading a trait), domain cards always kept at level 3+, inventory 'Create card' button-card, and weapons/armor in the archive.`n`nNote: enabling homebrew expansions inside character creation is still coming in a follow-up.`n`nRecent history: v0.9.8 = Golden Gear Edit, Favorites. v0.9 = multiclassing, Ranger companion.`n`nSideload: enable Install unknown apps, then open the APK."
