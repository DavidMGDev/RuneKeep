...
$ver  = 'v0.14.0'                       # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.14.0 - all 120 rulebook loot items, subclass families, Experience modifiers, and NFC sharing that keeps a card's art. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`n- LOOT + CONSUMABLES, COMPLETE: the 19 missing loot items are transcribed, so all 60 loot and all 60 consumables are here - and they're finally reachable, via new Loot and Consumables tabs in Add card from catalogue. Both get their own forged card: a banded treasure chest on dug earth for loot, an alchemist's flask on apothecary glass for consumables, each with its own plaque and the rulebook table roll printed on it.`n- EXPERIENCE MODIFIERS: the Honing Relic ('+1 to an Experience of your choice') finally works. Any card effect can now target one of your Experiences - it defaults to your first, and you can retarget it to any of them in that card's Modifiers panel. The Modifiers panel gained an Experiences section showing each one's total, its level-up advancements, and any relic boosting it.`n- MAKE AN EXPERIENCE FROM NEW CARD: 'Experience' joined the card-type picker. Pick it and the editor becomes the same experience editor you get at character creation and level up, and it creates a real Experience that level-up and the Honing Relic can both see.`n- LEVEL-UP DOMAIN CARD FIX: fixed the intermittent bug where levelling up handed you a DIFFERENT domain card than the one you picked. A cancelled swipe on the gear strip could permanently freeze the picker's idea of which card was centred; it now reads the deck's live position at the moment you press.`n- NFC SHARING KEEPS THE CARD: a weapon or armor shared from your Card Library now arrives with its real decoration - glyph, stat block, equipment plaque - instead of a flat colour, and uploaded card art travels with it instead of arriving blank. (Homebrew weapons already on your sheet get the proper card too.)`n- NFC RECEIVING FEELS RIGHT: your phone buzzes the instant a tag is read, before anything appears. The confirmation shows just the card's name (it used to render a card that bled out of the panel), and the card now eases down from off-screen over a longer, calmer arc instead of popping in.`n- SUBCLASS FAMILIES: custom subclass cards print Foundation / Specialization / Mastery under the title like the official ones, and the three cards link up automatically when they share a name (capitals don't matter) - no more upgrades that silently do nothing. Enabling or opening an expansion with an incomplete subclass now warns you, naming what's missing.`n`nSideload: enable Install unknown apps, then open the APK."
