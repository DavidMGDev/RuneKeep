...
$ver  = 'v0.10.0'                          # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.10.0 - Card Library & homebrew expansions, the .rkp file format, plus four gameplay bug fixes. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`nThis release:`n- NEW - Card Library: the main-menu Cards button now opens a real library. Browse the full system archive (every card, plus all weapons & armor across tiers 1-4), and author your own homebrew.`n- NEW - Expansions: create named, versioned bundles of homebrew cards. Each card can be an Ancestry (with mixed-ancestry line support), Community, Domain card (with a domain + level), Subclass, Class, or a generic card, authored with a live forged-card preview.`n- NEW - .rkp sharing: RuneKeep's own file format. Export/import characters, single cards, or whole expansions as .rkp files through the share sheet. Re-importing an expansion you already have UPDATES it in place when its version is newer - homebrew that travels.`n- FIX - Level up past level 8: trait-upgrade marks now reset at every tier (levels 2, 5, 8), and you can't upgrade the same trait twice in one level. The trait advancement can always be completed again - the level-9 soft-lock is gone.`n- FIX - Domain cards at level 3+: every domain card you pick on level-up now stays on your carousel. The 5-card limit only governs which are ACTIVE, never which you own.`n- FIX - Custom inventory items in creation: 'Create card' is now a button-card - each press makes a new, independently selectable item card (create several, pick the ones you want), with a separate Edit control.`n- FIX - Weapons & armor in the archive: all tiers are now browsable, with weapon/armor and tier filters.`n`nNote: enabling homebrew expansions inside character creation, and NFC tap-to-share, are coming in a follow-up. .rkp file sharing works today.`n`nRecent history: v0.9.8 = Golden Gear Edit, Favorites, sheet buttons. v0.9.7 = saved in-play state. v0.9 = multiclassing, Ranger companion.`n`nSideload: enable Install unknown apps, then open the APK."
