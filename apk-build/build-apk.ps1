...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.23.0 - .rkp files finally open from WhatsApp and file managers, artwork travels with shared characters, onboarding happens where you need it, and the whole UI adapts to tablets. Offline Android APK (arm64-v8a, $mb MB), all data bundled, no download needed.`n`n- OPEN .RKP FROM ANYWHERE: this genuinely did not work before. The APK builder never regenerated the native manifest, so v0.22.0's file association was silently dropped from the build. Fixed, along with the pattern escaping and the fact that a file shared from WhatsApp carries no filename to match on. RuneKeep now shows up in the app list when you tap a .rkp.`n- ARTWORK TRAVELS: sharing a character now carries their portrait and the art on every custom card. Sharing a card over NFC compresses the image to fit the tag instead of dropping it.`n- ONBOARDING WHERE YOU NEED IT: three short tours instead of one on first launch. A welcome, one when you first make a hero, and one when you first open a sheet. They are interactive - the hand really fans, and hold-to-equip will not let you past until you have felt it. The ? on the menu turns the tips back on.`n- TABLETS: the interface keeps its shape instead of stretching. Content holds a readable measure and centres, the card grid adds columns rather than blowing up each card, and the sheet's gold border keeps the proportions it was drawn with. Phone layout is completely unchanged.`n- DM FIXES: the encounter's buttons line up at one height, the quick stat wheel's cursor appears under your thumb, log entries can be dragged again, and setting a stat above someone's maximum now asks whether it is a bonus for this fight or a real raise.`n- OVERLAPS: the Select buttons no longer sit under the card, the traits screen stops pushing itself off screen, your keyboard stops covering Save, folders remember what you collapsed and can be renamed, characters can be renamed, and the Card Archive title stops colliding with its own buttons.`n- WRITING: every em dash is gone, and the copy is plainer throughout.`n`nSideload: enable Install unknown apps, then open the APK."
