...
$ver  = 'v0.17.0'                       # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.17.0 - DM Mode, third pass: the full Base Game adversary roster, a real Adversary Library, and a batch of feedback fixes. Offline Android APK (arm64-v8a, $mb MB), all data bundled, no download needed.`n`n- BASE GAME ADVERSARIES: all 139 Daggerheart core-rulebook adversaries are bundled with their full stat blocks - tier, type, difficulty, thresholds, HP/Stress, standard attack, motives, experience, and every feature. Browse them in a full-screen ADVERSARY LIBRARY, kept separate from your own saved adversaries, with card-archive-style filters (Tier, Type, damage, traits) and search; each row shows Tier, Type, HP and damage at a glance. An info panel explains how each stat and adversary type works.`n- SPAWN & CONFIGURE: open the library from an encounter's Adversaries or Allies row, pick an adversary, choose how many, and spawn unique copies (editing a spawned copy never touches the saved one). The config editor now holds EVERYTHING a base adversary has - a portrait image, type, tier, difficulty, attack, motives, experience and a full feature list - so a custom adversary can match anything in the book. Tap any adversary's portrait to view it fullscreen; new adversaries start at a full 10/10 HP. The card archive also links straight to the library.`n- LOG, SMOOTHER: the side panel slides in cleanly from the left with the whole background fading to dim in one motion (no more elastic snap or undimmed gaps). Notes get a grabber to reorder them; auto-logged stat changes read differently and hold-to-select on either kind gives Delete selected or Leave only selected, both confirmed.`n- FEEDBACK FIXES: multi-select controls now live at the BOTTOM of every list and selection is bold and obvious; allies turn a friendly teal and adversaries stay red as you convert them; sessions and encounters are renamed by holding them in the list; disabled parties show in the Sessions dropdown from the moment they're created and the 'unlocked' toast only fires for your first party.`n`nSideload: enable Install unknown apps, then open the APK."
