...
$ver  = 'v0.11.1'                          # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.11.1 - Golden Gear Edit, take two. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`nA big feel + look pass over Edit Mode, from on-device testing:`n- DESATURATED LOOK - entering Edit Mode now flashes the gears white and desaturates the whole frame (border + gears go light gray) so it's unmistakable you're in edit. Leaving flashes them back to gold.`n- THE ROW - the cards are now the same size as when you gear-scroll, just STRAIGHTENED into a neat row with a hair of space between them (no more curve).`n- A proper 'Edit Mode' banner sits mid-screen with a live 'X / Y Cards' count and a Deselect All.`n- DRAGGING TO REARRANGE - fully reworked: the cards you drag now glide up to your finger, a single ghost slot shows exactly where they'll land (even when dragging a whole stack), and on release the row makes room and the cards settle in - no more snapping or flicker. Edge auto-scroll ramps up harder near the screen edges.`n- THE ACTION WHEEL - now works just like the sheet's radial menu: hold a card, the wheel blooms, slide to an option and let go. All icons (no text), bigger, all the way around. Unfavorite is a crossed-out star; the NFC icon is clearer.`n- EXITING - TAP the gear to close the cards completely; HOLD the gear to drop back to the normal fanned hand.`n- Scrolling the gear a bit and then holding still no longer trips Edit Mode by accident.`n- Cleaner Add Card / Add Gear / Favorites buttons - proper chamfered frames that fill correctly when active.`n`nSideload: enable Install unknown apps, then open the APK."
