...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.22.0 - character state history, a rewritten DM interface, an optional guided tour, .rkp files that open from anywhere, and a long list of text, language and safety fixes. Offline Android APK (arm64-v8a, $mb MB), all data bundled, no download needed.`n`n- CHARACTER STATE: the float menu's north slot is now State. It keeps the modifier breakdown you already had, and adds a full timeline of everything that has happened to your character - every equip, card edit, reorder, rest and level up. Rewind to any point with a hold. Rapid changes fold into one entry, milestones stay pinned, and a restored character is checked rather than trusted.`n- CARD TRASH: cards you wrote and later deleted are recoverable from a Trash tab in the Cards menu.`n- GUIDED TOUR: an optional walkthrough that shows the gestures nothing on screen advertises - hold to equip, the two halves of every resource track, and Edit Mode on the cogs. Skippable from the first frame, and reopenable from the ? on the main menu.`n- OPEN .RKP FROM ANYWHERE: tap a character or expansion file in WhatsApp, Gmail, Drive or a file manager and it opens in RuneKeep. Nothing imports without asking, and a file that arrives while you are mid-session waits until you are free.`n- DM INTERFACE REWORKED: the desaturated mode is now actually desaturated - borders, dialogs, the number pad and loading screens included. 21 font sizes became 5, spacing follows one rule, Start is the biggest control on an encounter instead of a decorative icon, the adversary library is chamfered again, and every empty state teaches instead of shrugging.`n- CREATION KEEPS YOUR WORK: the draft survives leaving the screen, the back button, and closing the app. Progress is shown, and Forge tells you what is missing instead of sitting dead.`n- SAFER: levelling and resting now need a hold, like everything else destructive. Removing a party member, deleting an ally and replacing a character on import all confirm and say what is lost. The hold-to-confirm fill follows the chamfer instead of squaring off the corners.`n- ELVES: Celestial Trance grants a third downtime move, and correctly loses it under mixed ancestry when elf is the first pick.`n- TEXT FIXES: overlapping and clipped text throughout - the DM vitals row, panel titles, the HP readout at three digits, unbounded names feeding fixed headers, and OS font scaling.`n`nSideload: enable Install unknown apps, then open the APK."
