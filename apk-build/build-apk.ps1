...
$ver  = 'v0.10.2'                          # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.10.2 - a big character-creator + homebrew authoring update, plus two important level-up fixes. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`nLevel-up fixes:`n- FIX - advancement options now RESET each tier, so you can raise traits / take HP & Stress again at tiers 3 and 4 - reaching levels 9-10 is possible again. Characters already stuck are healed automatically on load.`n- FIX - upgrading your Subclass now actually ADDS the next subclass card (specialization, then mastery) to your loadout; older saves are backfilled.`n`nCharacter creator:`n- NEW - a Random button in every section rolls a valid pick (class, subclass, ancestry, community, domains, traits, weapons, armor).`n- NEW - Skip cards for weapons, armor, and inventory - start a hero with none.`n- Class cards now appear in the Card Archive with full multi-page browsing.`n`nHomebrew card authoring (Card Library):`n- NEW - pick the content type FIRST, then author.`n- NEW - Weapon, Armor, and Item (inventory) card types, plus a custom type label - weapons & armor carry real stats.`n- NEW - the description box is now add/delete/reorder sections with a Bold / Italic / bullet toolbar (no hand-typed markdown).`n- NEW - custom Ancestry cards get two feature lines and the mixed-ancestry passive selector.`n`nSharing:`n- NFC now shares a single card at a time (from the Library per card, or Golden Gear Edit) - simpler, and always within NFC's size limit.`n`nStill coming: SELECTING your homebrew ancestry/subclass/class/domain cards inside character creation (the authoring + data model shipped here; the creator + sheet wiring lands next).`n`nSideload: enable Install unknown apps, then open the APK."
