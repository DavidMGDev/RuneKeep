...
$ver  = 'v0.13.1'                          # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.13.1 - Martial Form, DM card sharing, and four fixes. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`n- MARTIAL FORM: the Brawler's Martial Artist subclass gets its own card category (the Beastform/Companion treatment) - all 24 stances from the printed Martial Form sheet, tier-gated to your level, one stance active at a time (mechanical stances like Steady and Immovable really modify your sheet), plus a live Focus card with tappable token pips for the d6s you roll at rest.`n- DM CARD SHARING: hold a fullscreened card in the Card Archive - it charges with the same gold fill as enabling a carousel card, then opens the NFC send panel. The receiving phone resolves the card against its own catalog, so official cards arrive with their real art; weapons and armor travel with full stats.`n- SCAR FIX: scarring a FILLED hope slot now depletes that hope with the usual explode animation (it used to stay lit under the scar).`n- TITLE FIX: long weapon/armor names (Legendary Arcane-Frame Wheelchair, the Gauntlets family) now shrink to fit instead of ending in '...'. All forged cards regenerate once on first launch.`n- VOID ANCESTRIES now appear in Add card from catalogue's Ancestry tab (they were the one Void content type missing there).`n`nSideload: enable Install unknown apps, then open the APK."
