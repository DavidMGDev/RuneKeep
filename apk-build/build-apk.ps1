...
$ver  = 'v0.9.8'                           # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.9.8 - Golden Gear Edit, Favorites, and new card-action buttons. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`nThis release:`n- NEW - Golden Gear Edit: press and HOLD the golden gear perfectly still for a moment and your card deck straightens into a flat row of small cards. Tap cards to raise/select them, then Duplicate, Move, Delete, or Favorite the whole selection from a bar at the top (with the same safeguards as the Cards panel), or jump straight to the Cards panel. Swipe a card UP to grab and drag it to a new spot; the others make room. Tap Done to finish.`n- NEW - Favorites: a star category of card duplicates. Favorite any card (in Golden Gear Edit or the Cards panel) and it's mirrored into Favorites - tokens, dice, edits, and enabled state stay in sync with the original; deleting a favorite just un-favorites it. Reach Favorites from the new star button on your sheet; it joins the over-scroll loop once you have one.`n- NEW - sheet buttons: the Ancestry / Community / Subclass badges are replaced by Add Card, Add Gear, and Favorites - the actions you actually use.`n- FIX - Gold: hold-to-bulk on the Gold card is rebuilt. It no longer stops after a few or runs away after fast taps; just hold to ramp up smoothly, release to stop.`n- NEW - portrait feedback: tapping your portrait gives a quick shine, and holding to replace it is twice as fast so it feels responsive.`n`nRecent history: v0.9.7 = saved in-play state + float-menu hit-detection + gold hold + gear-add audit. v0.9.6 = big performance pass. v0.9 = multiclassing, Ranger companion, group card moves.`n`nSideload: enable Install unknown apps, then open the APK."
