...
$ver  = 'v0.24.0'                       # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.24.0 - tablets now run the phone interface at a larger size, .rkp files open from WhatsApp for real this time, and the Hope and Fear ancestries have their correct text. Offline Android APK (arm64-v8a, $mb MB), all data bundled, no download needed.`n`n- TABLETS, PROPERLY: the app no longer rearranges itself on a big screen. It draws the phone interface into a phone-shaped frame, scaled up to fill the height and centred, with faint cards decorating the space either side. The gold border sits on a real edge again, nothing spills past it, dimming covers the whole screen instead of just the middle, and the traits step goes back to two rows of three. Phone layout is untouched.`n- .RKP FROM WHATSAPP: v0.23.0 got Android to open the app and then showed Unmatched Route, because the shared file arrives as a provider id rather than a filename. Both faults are fixed: the file is caught before the router sees it, and files with no name in them are accepted.`n- HOPE AND FEAR ANCESTRIES: all six were transcribed from a pre-release sheet. Aetheris and Gnome had the wrong second feature entirely, and four others had reworded mechanics. Retranscribed from the printed cards, with the flavour text they should have had.`n- CARD IMAGES: card previews are rebuilt on every update, so cards that were showing an old text-only face now show their artwork.`n- TITLE SCREEN: the drifting rows of cards actually loop. The middle one used to walk off the right of the screen, leaving a gap on the left that grew for two minutes and then snapped back.`n- SCREEN TITLES: no longer clipped by the top of the border.`n- ONBOARDING: the hand of cards demo matches the real carousel. The centre card sits on top where it belongs, and it plays itself rather than waiting for taps it never explained.`n`nSideload: enable Install unknown apps, then open the APK."
