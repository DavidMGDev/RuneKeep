...
$ver  = 'v0.16.0'                       # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.16.0 - DM Mode, second pass: usable heartbeats, fallen adversaries, an adversary library, and a lot of polish. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`n- HEARTBEAT CONTROLS, REWORKED: one global raise/lower toggle now sits in the screen corner, so every stat icon keeps its sheet colour (red hearts, gold armor, red stress bolt, amber hope) with no clutter. Icons are filled and GROW well above your fingertip while you hold them, the numbers are bigger, and the whole player panel is taller and tighter - HP, Armor, Stress and Hope grouped together. Expand a player to see traits, level, proficiency and their unique domain-card count.`n- ADVERSARIES DON'T VANISH: at 0 HP (or pressing the X) an adversary becomes 'Fallen' with a Recover button instead of being deleted - no more accidents. Real deletion goes through hold-to-select + confirm. Save any single adversary to a new ADVERSARY LIBRARY and spawn saved ones into a fight as an adversary or an ally NPC. Hold to multi-select adversaries, allies or encounters for bulk delete, move, duplicate, or convert allies to adversaries and back (players are always safe).`n- LOG, REBUILT: an animated 80%-width side panel. Edit notes after writing them, hold a note to move it earlier, and multi-select auto-logged stat changes to delete them.`n- FLOW POLISH: enabling a party toasts and turns its button straight into Sessions (plus a Sessions Menu button on the party list); a Party State button lets you set party vitals without an encounter; starting a second encounter asks first (and completes the other, noting it in its log); completed encounters can Restart from party or encounter state, reviving downed adversaries; the options panel gained Rename encounter and Open card archive; pop-ups are opaque and no longer close when you tap inside them; absent players read as greyed and Absent.`n`nSideload: enable Install unknown apps, then open the APK."
