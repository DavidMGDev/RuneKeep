...
$ver  = 'v0.14.1'                       # release version: bump here once -> drives tag, APK name, title
...
Set-Location $repo
$tag = $ver
$notes = "RuneKeep v0.14.1 - loot follow-ups: potions that actually work, loot in the archive, and spent consumables. Offline Android APK (arm64-v8a, $mb MB), all card data bundled, no download needed.`n`n- POTIONS AND RELICS NOW APPLY THEIR BONUS: every loot and consumable that promises a numeric bonus to a sheet stat now carries a real modifier, applied the moment you toggle the card on. The Major potions (+1 Agility/Strength/Finesse/Instinct/Presence/Knowledge until your next rest), the six basic potions, and the Shrinking (+2 Agility, -1 Proficiency) and Growing (+2 Strength, +1 Proficiency) potions all shipped doing nothing. Duration is yours to manage - toggle the card off when it runs out.`n- LOOT IN THE CARD ARCHIVE: the archive now browses all 60 loot and all 60 consumables, with Loot and Consumables filter chips, and you can hold one to share it over NFC like any other card. A shared loot item arrives on the other phone as REAL loot - its own chest/flask card with working modifiers - not a flattened note.`n- SPENT CONSUMABLES: toggling a consumable off now asks whether you used it up, and offers to discard the depleted card (hold to confirm). It only ever offers - if you're carrying several of the same potion it removes just the one.`n`nSideload: enable Install unknown apps, then open the APK."
